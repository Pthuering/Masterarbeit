...
Die theoretische Perspektive betrachtet Fine-Tuning als Optimierungsproblem mit multiplen Constraints. Keine universelle Lösung existiert: Trade-offs zwischen verschiedenen Zielen sind unvermeidbar. PEFT-Methoden wie LoRA verschieben Trade-offs, eliminieren sie aber nicht. Empirische Validierung ist unerlässlich für praktische Anwendungen.
    ...
    % Ziel: 2-2,5 Seiten
        ...
        Automatische Metriken für Natural Language Processing umfassen verschiedene Ansätze. BLEU und ROUGE sind N-gram Overlap-basierte Metriken, die ursprünglich für Maschinenübersetzung entwickelt wurden. Sie sind limitiert für semantische Äquivalenz, aber schnell berechenbar, wenngleich oberflächlich.

Semantische Ähnlichkeitsmetriken wie BERTScore nutzen Contextualized Embeddings statt Surface-Form. Dies ermöglicht bessere Erfassung von Paraphrasen, ist jedoch rechenintensiver, aber aussagekräftiger.

Perplexity misst die Konfidenz des Modells. Niedriger Wert bedeutet besseres Language Modeling. Dies ist nicht direkt für Qualität geeignet, aber für Training-Monitoring relevant.

Prompt-basierte Evaluation [SOURCE-MISSING] nutzt LLM-as-a-Judge durch größere Modelle zur Bewertung. Prompts wie „Bewerte die Qualität dieser Transformation auf Skala 1-5" definieren Kriterien wie Faktentreue, Verständlichkeit und Stilkonformität. Der Vorteil liegt in Flexibilität und domänenspezifischer Anpassbarkeit. Limitierungen bestehen in Bias des Evaluator-Modells und Kosten.

Comparative Evaluation [SOURCE-MISSING] führt A/B-Vergleich zweier Outputs durch. Fragen wie „Welcher Text ist verständlicher: A oder B?" sind robuster als absolute Bewertungen. Dies ist nützlich für Prompt-Optimierung (siehe Abschnitt~\ref{sec:2.2.2}).

Domänenspezifische Metriken umfassen Faktentreue durch Prüfung gegen strukturierte Eingabedaten. Entitätsextraktion erfasst Liniennummer, Haltestellen und Zeitangaben. Vollständigkeitsprüfung stellt sicher, dass alle Input-Informationen im Output enthalten sind. Halluzination Detection identifiziert erfundene Details (siehe Abschnitt~\ref{sec:2.4.2}).

Stilkonformität überwacht Einhaltung von Stilrichtlinien (siehe Abschnitt~\ref{sec:3.2}) durch regelbasierte Checks wie Passiv versus Aktiv und Fachbegriffe versus Allgemeinsprache. Pattern-Matching identifiziert verbotene Formulierungen.

Konsistenz stellt sicher, dass gleiche Eingabe zu identischer Ausgabe führt bei deterministischer Temperatur. Messung erfolgt durch wiederholte Generierung und Hamming-Distanz zwischen Outputs. Dies ist wichtig für Produktionsumgebung zur Vorhersagbarkeit.
        ...
        Halluzinationen bezeichnen faktisch inkorrekte oder erfundene Informationen, die Modelle generieren. Dies ist besonders kritisch bei sicherheitsrelevanten Verkehrsinformationen. Die Herausforderung besteht in plausibel klingenden, aber falschen Aussagen. Ein Beispiel ist „Straßenbahn Linie 10" statt korrekter „Bus Linie 10".

Self-Consistency Checks [SOURCE-MISSING] basieren auf mehrfacher Generierung mit Sampling (siehe Abschnitt~\ref{sec:2.2.2}). Der Mechanismus generiert N verschiedene Outputs für gleiche Eingabe, typisch N=3-10. Sampling erfolgt mit Temperature größer 0 für Variabilität. Mehrheitsentscheidung oder Konsensanalyse folgen. Die Annahme ist, dass korrekte Antworten konvergieren, während Halluzinationen divergieren. Der Vorteil besteht darin, dass keine externe Wissensquelle erforderlich ist. Die Limitation liegt in mehrfachen Inferenz-Kosten mit N-fachem Rechenaufwand. Anwendung findet dies bei kritischen Transformationen, wo Faktentreue essentiell ist.

Fact Checking gegen Eingabedaten erfolgt durch strukturierte Validierung [SOURCE-MISSING]. Extraktion von Entitäten aus Input und Output ermöglicht Vergleich: Sind alle Input-Entitäten im Output enthalten? Die Regel lautet, dass Output keine Entitäten enthalten darf, die nicht im Input sind.

Named Entity Recognition (NER) extrahiert Liniennummern, Haltestellen, Zeitangaben und Fahrzeugtypen durch Pattern-Matching oder NER-Modell.

Constraint-basierte Validierung nutzt Whitelist-Ansatz, wobei nur bekannte Liniennummern erlaubt sind. Knowledge Base mit Linien-Fahrzeug-Zuordnungen als Ground Truth führt zu Reject-Output bei Verletzung von Constraints.

Prompt-basierte Halluzination Prevention [SOURCE-MISSING] verwendet negative Constraints in System Prompt (siehe Abschnitt~\ref{sec:2.2.2}) wie „Erfinde KEINE Details, die nicht in der Eingabe stehen", „Nutze NUR Informationen aus dem bereitgestellten Kontext" und „Bei Unsicherheit: NICHT spekulieren, sondern Information weglassen".

Knowledge Grounding bettet Fakten explizit im Prompt ein. Beispiel: „Linie 10 ist ein Bus, Linie 11 ist eine Straßenbahn" reduziert Halluzinationen durch direkte Verfügbarkeit von Wissen.

Chain-of-Thought für Faktentreue nutzt Zwischenschritt: „Welche Informationen sind in der Eingabe enthalten?" Explizite Verifikation vor Generierung erhöht Token-Kosten, verbessert aber Zuverlässigkeit.

Automatisierte versus manuelle Evaluation unterscheidet skalierbare, aber limitierte automatisierte Ansätze von präziser, aber teurer manueller Evaluation. Automatisiert sind Entitätsvergleich (schnell, deterministisch), Pattern-Matching (regelbasiert, wartungsintensiv) und Self-Consistency (rechenintensiv, keine externe Referenz). Manuell sind Human Evaluation als Gold Standard (nicht skalierbar) und stichprobenbasierte regelmäßige Quality Checks mit iterativer Verbesserung durch Fehleranalyse zu Prompt-Anpassung.

Hybrid-Ansatz ist optimal für Praxis durch automatisierte Pre-Filter für offensichtliche Fehler, manuelle Review für Grenzfälle und Stichproben sowie Feedback-Loop, wobei erkannte Fehler zu Prompt-Optimierung führen.

Praktische Ansätze für ressourcenlimitierte Umgebungen nutzen deterministische Temperatur (T=0) zur Reduktion von Halluzinationen [SOURCE-MISSING]. Der Trade-off besteht in weniger Kreativität, aber höherer Konsistenz. Dies ist geeignet für faktische Transformationen.

Kleinere Validierungsmodelle umfassen leichtgewichtige NER-Modelle für Entitätsextraktion sowie regelbasierte Checks statt LLM-as-a-Judge.

Cached Knowledge Integration verwendet statische Wissensbasis im Prompt ohne RAG-Overhead. Siehe Abschnitt~\ref{sec:2.3.3} für Knowledge Integration ohne RAG.

Der Übergang zu Kapitel~\ref{chap:7} verdeutlicht, dass theoretische Konzepte hier Methodik und Prinzipien darstellen. Praktische Anwendung in Kapitel~\ref{chap:7} zeigt konkrete Metriken und Ergebnisse. Kombination automatisierter und manueller Validierung erfolgt in Implementierung.
